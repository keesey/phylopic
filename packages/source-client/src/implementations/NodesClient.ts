...
import { isNonemptyString, UUID } from "@phylopic/utils"
...
class NodeSearchClient implements Listable<Node & { uuid: UUID }, number> {
    ...
    async page(index = 0) {
        ...
        const output = await client.query<Node & { uuid: UUID }>(
            'SELECT "uuid", created, modified, "names"::json, parent_uuid FROM (SELECT uuid, created, modified, "names"::character varying, parent_uuid, json_array_elements(json_array_elements("names"))->>\'text\' ILIKE $4::character varying AS "match" FROM node ORDER BY "match" DESC LIMIT $1::bigint) AS matches WHERE "match"=true GROUP BY "uuid", created, modified, "names", parent_uuid ORDER BY "names"::character varying OFFSET $2::bigint LIMIT $3::bigint',
