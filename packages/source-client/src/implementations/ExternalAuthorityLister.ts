...
export class ExternalAuthorityLister implements Listable<Authority, number> {
    ...
    async page(index = 0): Promise<Page<string, number>> {
        ...
        const output = await client.query<{ authority: Authority }>(
            `SELECT authority FROM ${EXTERNAL_TABLE} WHERE disabled=0::bit GROUP BY authority,disabled ORDER BY authority OFFSET $1::bigint LIMIT $2::bigint`,
    ...
    public async totalItems() {
        ...
        const output = await client.query<{ total: number }>(
            `SELECT COUNT(DISTINCT authority) AS total FROM ${EXTERNAL_TABLE} WHERE disabled=0::bit`,
        )
