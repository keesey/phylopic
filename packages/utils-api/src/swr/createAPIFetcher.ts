...
export const createAPIFetcher =
    <T extends Readonly<{ build: number }>>(build: number, setBuild?: Dispatch<SetStateAction<number>>): Fetcher<T, URL> =>
    async key => {
        try {
            const response = await axios.get<T>(key, { responseType: "json" })
            const dataBuild =response.data?.build
            if (typeof dataBuild === "number" && dataBuild > build) {
            ...
            if (axios.isAxiosError(e) && e.response) {
                if (e.response.headers["content-type"] === DATA_MEDIA_TYPE) {
                    ...
                    if (typeof dataBuild === "number" && build < dataBuild) {
                        setBuild?.(build = dataBuild)
                    }
