...
Precomputed entity and list JSON for the public API, written during publish builds. Accessible via `https://entities.phylopic.org` (once CloudFront is configured).

Lineage (`GET /nodes/{uuid}/lineage`) and resolve (`GET /resolve/...`) are **not** stored here; the API serves those from Postgres (`node` and `node_external`).
...
- `{build}/`
    ...
    - `lists/`
        - `{contributors|nodes|images}/`
            ...
            - `pages/`
                - `{page}.json` - Page of titled links (no embedded items). Requests with `embed_items=true` are served from Postgres.

Each object mirrors the corresponding Postgres data for that build. Objects are immutable (`Cache-Control: public, max-age=31536000, immutable`).
